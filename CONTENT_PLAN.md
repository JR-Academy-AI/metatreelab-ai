...
3. Write ONE article as a single markdown file at `src/content/insights/<slug>.md`, English only, with full frontmatter:
   ...
   tags: ["...", "...", "..."]
   author: "Lightman Wang"     # E-E-A-T byline — resolves to the siteConfig.author
                               # profile (LinkedIn sameAs, job title, bio, author card).
                               # Keep this name; it is also the schema default.
   draft: false
   ---
   ```
...
6. Run `pnpm check && pnpm build` (both must pass — the earlier `src/data/projects.ts` / `ProjectThumbnail.astro` type errors are now fixed, so `pnpm check` should report 0 errors).
