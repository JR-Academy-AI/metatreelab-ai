...
| 7 | ❌ **禁止 skill 渲染 HTML / PDF / Next.js 物料站** | skill 自己启动 puppeteer / 修改 land-iq-pitch repo | skill 只产 markdown 内容到 `proposals/{slug}-{date}/`。任何 HTML/PDF/物料站渲染都属于手动后续工序 — 在 Step 5 给用户**指令**而不是替他执行 |
...
10. **proposal 语言** + **交付格式**：中文 / 英文 / 双语；只要 .md / 还是要 PDF。默认中文 .md。
...
- [ ] §02 Scope (Out) 非空，至少 3 条 explicit exclusion
- [ ] §08 法律条款没改基础默认值（Queensland 法 / net-14 / 60 天 / liability cap = fees paid）
